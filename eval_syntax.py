...
# -*- coding: utf-8 -*-
"""
Evaluation with models' MLM heads involved
@author: ZhiruoWang
"""
...
def create_dataset(test_path, tokenizer, seq_length=128):
    ...
    with open(test_path, "r", encoding='utf-8') as fr:
        for line in fr:
            query, position = line.strip().split('\t')
            answer = query[int(position)]
            instance = tokenizer.tokenize_for_eval(
                query=query, answer=answer, position=position, 
...
def main():
    ...
    parser.add_argument("--topk",type=int, default=20,  help="Top k choice for cloze tasks.")

    # io options    
    parser.add_argument("--data_path", type=str, default="./data/knowledge/syntax/adverb.txt")
    ...
    for i, (query_batch,type_batch,attn_batch,label_batch) in enumerate(
        load_batch(args.batch_size, query_ids, token_type_ids, attn_mask, mlm_labels)
    ):
        with torch.no_grad():
